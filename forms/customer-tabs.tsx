...
const schema = yup
  .object({
    ...
    govID: yup.string().test(
      'is-cnpj', // Test name
      errorMessage.cnpj.invalid, // Error message
      function(value) {
        return !!value && cnpj.isValid(value); // Accessing other field value
      }
    ).transform(value => getOnlyDigit(value)).required(errorMessage.cnpj.required),
  ...
  return (
    ...
    <form onSubmit={handleSubmit(handleData)}>
      ...
      <Tab title="Dados pessoais">
        <div className="form-row">
          <Input {...register("name")} defaultValue={falsy(entity?.name)} label="Nome" placeholder="Digite o nome empresarial" isClearable isInvalid={!!errors.name} color={getInputColor(errors.name)} errorMessage={getInputErrorMessage(errors.name)} />
          <Input {...register("govID")} defaultValue={falsy(entity?.govID)} label="CNPJ" placeholder="Digite o CNPJ" isClearable isInvalid={!!errors.govID} color={getInputColor(errors.govID)} errorMessage={getInputErrorMessage(errors.govID)} />
    ...
    {operation === 'update' && (
      ...
      {operation === 'update' && (
        <Tab title="Demais">
          <div className="form-row">
            <Input {...register("info.name")} defaultValue={falsy(info?.name?.toString())} label="Nome Fantasia" placeholder="Digite o nome fantasia" isClearable />
            <Input {...register("info.name_in_charge")} defaultValue={falsy(info?.name_in_charge?.toString())} label="Nome do Responsável" placeholder="Digite o nome do responsável" isClearable />
            <Input {...register("info.phone")} type="tel"  defaultValue={falsy(info?.phone?.toString())} label="Contato do Responsável" placeholder="Digite o celular do responsável" isClearable />
          </div>
          <ModalFormFooter isLoading={isHandlingMutation} buttonLabel={buttonLabel} handleClose={handleClose} />
        </Tab>
      )}
