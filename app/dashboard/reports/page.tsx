              ...
              <li key={user.id}>
                ...
                <ul className='ml-2'>
                  {user.turn.map(turn =>
                    <li key={turn.id}>
                      <Card className='p-4 my-4'>
                    <h3>Turno: {new Intl.DateTimeFormat('pt-BR', 
                      {
                        year: "numeric",
                        month: "numeric",
                        day: "numeric",
                      ...
                      }).format(new Date(turn.startedAt))} - {turn.endedAt && new Intl.DateTimeFormat('pt-BR', 
                      {
                        year: "numeric",
                        month: "numeric",
                        day: "numeric",
                        ...
                        hour12: false,
                        timeZone: "America/Sao_Paulo",
                      }).format(new Date(turn.endedAt))}</h3>
                    <ul className='ml-4'>
                      {turn.operation.map(operation =>
                        <li key={operation.id}>
                          <h4 className='text-sm'>Operação <b>{operation.type}</b>: {new Intl.DateTimeFormat('pt-BR', 
                      {
                        year: "numeric",
                        month: "numeric",
                        day: "numeric",
                        ...
                        hour12: false,
                        timeZone: "America/Sao_Paulo",
                      }).format(new Date(operation.startedAt))} - {operation.endedAt && new Intl.DateTimeFormat('pt-BR', 
                      {
                        year: "numeric",
                        month: "numeric",
                        day: "numeric",
                        ...
                        hour12: false,
                        timeZone: "America/Sao_Paulo",
                      }).format(new Date(operation.endedAt))}
                      ...
                      <span className='mx-2'>|</span>Total de Viagens: <b>{operation.travel.length}</b></h4>
                              ...
                              <TableBody items={operation.travel}>
                                {(item) => (
                                  <TableRow key={item.id}>
                                    {(columnKey) => <TableCell>{getKeyValue(item, columnKey)?.toString().includes("-") ? new Intl.DateTimeFormat('pt-BR', 
                                      {
                                        hour: "numeric",
                                        minute: "numeric",
                                        second: "numeric",
                                        hour12: false,
                                        timeZone: "America/Sao_Paulo",
                                      }).format(new Date(getKeyValue(item, columnKey))) : getKeyValue(item, columnKey)}</TableCell>}
                                  </TableRow>
                                )}
                              </TableBody>
                            </Table>
